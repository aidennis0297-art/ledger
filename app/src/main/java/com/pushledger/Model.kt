...
data class Raw(
    ...
    /**
     * 돈이 걸린 알림인지. 보관 기간을 여기서 가른다.
     *
     * 규칙이 왜 못 읽었는지는 **원문을 봐야** 알 수 있고 원문은 이 앱 안에만 있다.
     * 잡담과 같은 칸에 묶여 있으면 앱을 가볍게 하려고 기간을 줄일 때 고칠 근거까지
     * 같이 지워진다. `명한돌` 오분류가 원문이 없어 아직 못 고친 건이다.
     *
     * **버릴 것을 세는 쪽으로 적었다.** 남길 것을 나열하면 상태가 새로 생겼을 때
     * 그 목록에 넣는 것을 잊고, 새 상태의 원문이 조용히 일찍 사라진다. 반대로 적으면
     * 잊었을 때 오래 남을 뿐이라 잃는 것이 없다 — 틀릴 거면 이쪽으로 틀려야 한다.
     */
    val isMoney: Boolean get() = state != IGNORED && state != NOISE
}
...
data class Config(
    ...
    /** 잡담 알림 보관 기간. 목록을 덮는 쪽이라 짧게 둔다. */
    val keepInboxDays: Int = 30,
    /**
     * 돈이 걸린 알림 원문 보관 기간. [keepInboxDays] 보다 짧게는 안 잡힌다.
     *
     * 규칙이 왜 못 읽었는지는 원문을 봐야 알 수 있고 원문은 이 앱 안에만 있다.
     * 잡담과 같은 칸에 묶여 있던 탓에, 앱을 가볍게 하려고 기간을 줄이면 고칠 근거까지
     * 같이 사라졌다. 90일이면 한 분기치 오분류를 모아 한 번에 볼 수 있다.
     */
    val keepMoneyDays: Int = 90,
    /**
