            ...
            r.onSuccess { res ->
                when {
                    ...
                    res.category == Cat.INCOME &&
                        ...
                        Store.setRawState(id, Raw.IGNORED, "수입은 기록하지 않습니다 (${won(res.amount)}) · 넣으려면 직접 입력")
                        ok++
                    }
                    res.amount <= 0L -> {
                        ...
                        if (added) {
                            Store.setRawState(id, Raw.DONE, "AI: ${res.merchant} ${won(res.amount)}")
                        } else {
                            Store.setRawState(id, Raw.IGNORED, "AI: 이미 기록된 결제 (${won(res.amount)})")
                        }
                        ok++
                    }
                }
