 ...
 *   **다만 입금 알림은 거래로 자동 기록하지 않는다**(HANDOFF 불변식 15). 그래서 이 값은
...
object Stats {
     ...
     * 넉넉하게 계산된다.
     *
     * 판정은 네 가지를 다 넘겨야 한다. 서로 다른 달에 [minMonths] 번 이상 나올 것,
     * 그 달에 한 번씩만 나올 것, 금액이 중앙값에서 20% 안일 것, 그리고 이웃한 결제
     * 사이의 간격이 25~35일일 것.
     *
    ...
    fun recurring(list: List<Txn>, cfg: Config, minMonths: Int = 3): List<Recurring> {
        ...
        return active(list)
            ...
            .mapNotNull inner@{ (k, txns) ->
                ...
                // 결제 간격이 한 달 언저리가 아니면 구독이 아니다.
                //
                // 개수와 금액만 보면 "석 달 연속 비슷한 값을 쓴 단골 식당" 이 구독으로 잡힌다.
                // 반복 결제를 가려내는 업계 방식도 금액·가맹점과 함께 주기의 규칙성을 본다.
                //
                // 날짜 숫자(3일, 29일)를 빼지 않고 **실제 날짜 사이의 일수**를 센다. 그래야
                // 말일 구독이 저절로 풀린다 — 7월 31일과 8월 30일은 날짜로는 하루 차이지만
                // 실제로는 30일 간격이고, 그게 우리가 보려는 값이다. 날짜 숫자로 재면
                // 달을 넘나드는 경우마다 예외를 손으로 붙여야 하고 그 예외가 또 틀린다.
                //
                // 25~35일로 잡는다. 달 길이가 28~31일이고 결제일이 주말이면 며칠 밀린다.
                val dates = picks.map { at(it).toLocalDate() }.sorted()
                val gaps = dates.zipWithNext { a, b -> java.time.temporal.ChronoUnit.DAYS.between(a, b) }
                if (gaps.any { it !in 25L..35L }) return@inner null
