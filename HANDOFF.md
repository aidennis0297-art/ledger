...
소스 32개 파일 9,933줄 + 테스트 7개 파일 1,041줄. **유닛 테스트 91개 전부 통과.**
...
### 아직 남은 것 (같은 기록에서 나온 것)

고친 것 (전부 그 기록에서 나왔고 테스트에 원문 그대로 들어 있다):
...
- **술집·식당 이름(`금솥` `호맥 예천점` `생생호프`)이 기타로 간다.** 상호만으로는
  업종을 알 수 없다. 사용자가 한 번 고치면 `catMemory` 가 기억하므로 규칙을
  늘리기보다 그쪽이 맞다.
