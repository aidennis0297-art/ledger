...
소스 32개 파일 9,933줄 + 테스트 7개 파일 1,041줄. **유닛 테스트 86개 전부 통과.**
...
- **편의점 결제 7건이 `기타/기타지출` 로 들어간다.** `씨유 휘경행복점` 이 분류 규칙에
  없다. 항목별 예산을 쓰는 사람에게는 이게 곧 예산이 틀리는 것이다.
- **`확인해주세요` 38,000원이 수입으로 잡혔다.** 원문은 카카오톡 정산 요청
  (`정산 내용을 확인해주세요. - 정산금액 : 38,000원 - 요청인원 : 4명`)이다.
  **내가 낼 돈을 달라는 요청**인데 수입이 됐고, 가맹점 자리에 문구가 들어갔다.
- **`전자금융거래약관` 2,200원** — G마켓 스마일캐시 소멸 안내가 지출로 잡혔다.
  `소멸 예정` 은 돈이 나가는 것이 아니다.

- **`아크(ARK)PC방` 이 `PC방` 으로 줄어든다.** 괄호를 지운 뒤 토큰 중 가장 긴 것을
  고르는 규칙 때문에 `아크` 가 떨어진다. 돈은 맞고 이름만 뭉툭하다.
