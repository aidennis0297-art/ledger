...
소스 32개 파일 9,583줄 + 테스트 6개 파일 825줄. **유닛 테스트 60개 전부 통과.**
...
- 접는 순서는 **아래에서부터**다. 월말 예상 → 도트 둘째 줄 → 도트 → 오늘 지출·한도 줄.
  큰 숫자와 배지는 1x1 에서도 남는다.
...
- **월말 예상 지출을 홈 화면에도** — `현재 지출 ÷ 경과일 × 총일수`.
  예산을 넘긴 뒤가 아니라 넘기기 전에 알려 준다. safe-to-spend 계열 앱의 기본 기능.
  **위젯에는 이미 들어갔다**(설정 › 홈 위젯에서 켜고, 칸이 넉넉할 때 뜬다).
  계산이 한 줄이라 홈에도 같은 식을 쓰면 된다 — 두 곳이 다른 숫자를 말하지 않게 할 것.
