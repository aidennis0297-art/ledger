...
소스 32개 파일 9,933줄 + 테스트 7개 파일 1,041줄. **유닛 테스트 78개 전부 통과.**
...
- **진짜 버그는 다른 데 있었다 — 삼성페이 결제 18건 242,800원이 통째로 빠져 있었다.**
  삼성 월렛은 제목을 `₩7,100 결제 완료`, 본문을 `씨유 휘경행복점` 으로 보낸다.
  `Parser.MONEY` 가 `원` 글자만 보던 탓에 전부 '금액 못 찾음' 으로 떨어졌다.
  삼성페이는 허용 앱 목록에 처음부터 있었는데 **기록된 건이 한 건도 없었다** —
  허용 목록에 있으니 됐겠거니 하고 아무도 세어 보지 않은 것이다. 지금은 읽는다.
...
### 4-12. 금액 표기는 `원` 만이 아니다
삼성 월렛(삼성페이)은 `₩7,100 결제 완료` 로 보낸다. **원 기호가 앞에 붙고 '원' 글자가 없다.**
`Parser.MONEY` 가 `원` 만 보던 탓에 편의점·PC방·마트 결제 18건 24만원이 통째로 빠져 있었다.
허용 앱 목록에 있다고 잡히는 것이 아니다 — **앱마다 실제로 기록된 건수를 세어 볼 것.**
